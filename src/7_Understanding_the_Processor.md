...
（[英文版本](https://marabos.nl/atomics/hardware.html)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#instructions)）
...
> **汇编语言简介**
>
  当编译任何以编译语言（比如 rust 或者 C）编写的软件的时候，你的代码将会翻译成可以被处理器执行的*机器指令*，最终处理器将会以此执行你的程序。这些指令高度特定于你编译程序的处理器架构。

  这些机器指令也被称作*机器码*，是以二进制格式编码的，对于人类来说，完全可读。*汇编*是人类可阅读的代表。每一个指令用一行文本表示，通常以一个单词或者缩写表示指令，后面再跟上它的参数或者操作数。汇编器（`assembler`）将一段汇编文本转换成二进制表示，反汇编器（`disassembler`）则相反。
...
```
  ...
  对于特定内存地址的常量，例如上面示例中的 -5 和 1234，经常是以人类更容易阅读的*标签*来代替。在将汇编转换成机器码的时候，汇编器会自动将他们替换为实际的地址。
...
（[英文版本](https://marabos.nl/atomics/hardware.html#load-store-ops)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#lock-prefix)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#x86-cas)）
...
以下是 Rust 中等效的「比较并交换」循环的样子，就像我们在[第2章的“比较并交换操作”](./2_Atomics.md#比较并交换操作)中看到的那样：
...
（[英文版本](https://marabos.nl/atomics/hardware.html#load-linked-and-store-conditional-instructions)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#arm-load-exclusive-and-store-exclusive)）
...
>
  ...
  它还包括一个与 `com⁠pare_​exchange` 相对应的 cas（比较并交换）指令。当使用此指令时，`compare_exchange` 和 `compare_exchange_weak` 之间没有差别，就像在 x86-64 上一样。
...
（[英文版本](https://marabos.nl/atomics/hardware.html#compare-and-exchange-on-arm)）
...
> **「比较并交换」循环的优化**
>
  正如我们在[x86-比较并交换指令](#x86-比较并交换指令)中看到的，在 x86-64 上，`fetch_or` 操作和等效的 `compare_exchange` 循环编译成了完全相同的指令。人们可能期望在 ARM 上也会发生同样的情况，至少在 `compare_exchange_weak` 上，因为加载和「弱比较并交换」操作可以直接映射到 LL/SC 指令。
...
（[英文版本](https://marabos.nl/atomics/hardware.html#caching)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#cache-coherence)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#the-write-through-protocol)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#mesi)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#performance-experiment)）
...
```
  ...
  这意味着，对于我们在[第四章](./4_Building_Our_Own_Spin_Lock.html)的 SpinLock 中不使用 compare_exchange（或 swap）来自旋循环可能更高效，而是首先使用 load 操作去检查锁是否已经释放锁。那样，我们可以避免不必要地声明相关缓存行的独占访问权限。
...
另一方面，当多个（原子）变量相关并且经常快速连续访问时，将它们放在一起可能是有益的。例如，我们在[第4章](./4_Building_Our_Own_Spin_Lock.md)中的 `SpinLock` 将 T 紧挨着 AtomicBool 存储，这意味着包含 AtomicBool 的缓存行也可能包含 T，因此对一个（独占）访问的声明也包括了另一个。这是否有益完全取决于情况。

## 重排

（[英文版本](https://marabos.nl/atomics/hardware.html#reordering)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#memory-ordering)）
...
>
  ...
  我们在[第 3 章](./3_Memory_Ordering.html)中讨论的理论内存模型为此类处理器架构留出了空间，因为它不要求除顺序一致的原子操作之外的任何操作具有全局一致的顺序。
...
（[英文版本](https://marabos.nl/atomics/hardware.html#x86-64-strongly-ordered)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#arm64-weakly-ordered)）
...
>
  ...
  正如我们在 [ARMv8.1 原子指令](#armv8-1-原子指令)讨论的，ARM64 的 ARMv8.1 版本包括 CISC 风格的原子操作指令，如 ldadd（load 和 add）作为 ldxr/stxr 循环的替代。
...
（[英文版本](https://marabos.nl/atomics/hardware.html#reordering-experiment)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#memory-fences)）
...
（[英文版本](https://marabos.nl/atomics/hardware.html#summary)）
...
图 7-1。各种原子操作在 ARM64 和 x86-64 上编译为每个内存排序的指令概述。
  ...
  [下一篇，第八章：操作系统原语](./8_Operating_System_Primitives.html)
