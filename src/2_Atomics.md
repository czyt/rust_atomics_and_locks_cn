...
后台线程现在使用 `Instant::now()` 和 `Instant::elapsed()` 去衡量它们在 `process_item()` 所花费的时间。原子的递增操作用于将微秒数递增到 total_time，并且原子的 max 操作用于跟踪 max_time 中的最高测量值。
...
* *weak* 版本「比较并交换」稍微更有效。
  ...
  [下一篇，第三章：内存排序](./3_Memory_Ordering.html)
